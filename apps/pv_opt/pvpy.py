...
class PVsystemModel:
    ...
    def optimised_force(self, initial_soc, static_flows, contract: Contract, **kwargs):
        ...
        prices = prices.set_axis(
            [t for t in contract.tariffs.keys() if contract.tariffs[t] is not None],
            axis=1,
        )
        if not use_export:
            if log:
                            ...
                            if round(cost_at_min_price, 1) < round(max_import_cost, 1):
                                for slot, factor in zip(window, factors):
                                    ...
                                    slot_available_capacity = max(
                                        ((100 - x["soc_end"].loc[slot]) / 100 * self.battery.capacity) * 2 * factor,
                                        0,
                                    )
                                    min_power = min(
                                        slot_power_required,
