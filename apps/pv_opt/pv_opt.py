...
DEFAULT_CONFIG = {
    ...
    "solar_forecast": {
        "default": "Solcast",
        "attributes": {
            "options": ["Solcast", "Solcast_p10", "Solcast_p90", "Weighted"]
        },
        "domain": "select",
    },
...
class PVOpt(hass.Hass):
    ...
    def optimise(self):
        ...
        if self.debug:
            ...
            self.log(
                f">>> Original: {soc_last_day.loc[soc_last_day.loc[: self.static.index[0]].index[-1] :]}"
            )
        ...
        try:
            ...
            soc_last_day = soc_last_day.loc[
                soc_last_day.loc[: self.static.index[0]].index[-1] :
            ]
            if self.debug:
                self.log(
                    f">>> Fixed   : {soc_last_day.loc[soc_last_day.loc[: self.static.index[0]].index[-1] :]}"
                )
                ...
                if (
                    ...
                    if self.hold and self.hold[0]["active"]:
                        if (
                            ...
                            or status.get("hold_soc", {}).get("soc", 0)
                            != self.hold[0]["soc"]
                        ):
                            self.log(
    ...
    def load_consumption(self, start, end):
        ...
        if self.get_config("use_consumption_history"):
            ...
            if (len(self.zappi_entities) > 0) and (
                self.get_config("ev_charger", "None") == "Zappi"
            ):
