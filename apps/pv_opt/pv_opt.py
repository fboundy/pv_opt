...
class PVOpt(hass.Hass):
    ...
    def optimise(self):
        ...
        self.pv_system.soc_now = self.get_config("id_battery_soc")
        soc_last_day = self.hass2df(self.config["id_battery_soc"], days=1, log=self.debug)
        if self.debug and "S" in self.debug_cat:
            self.log(f">>> soc_now: {self.pv_system.soc_now}")
        ...
        try:
            self.pv_system.soc_now = float(self.pv_system.soc_now)

        except:
            self.log("")
            self.log("Unable to get current SOC from HASS. Using last value from History.", level="WARNING")
            self.pv_system.soc_now = soc_last_day.iloc[-1]
        ...
        try:
            ...
            soc_last_day = pd.concat(
                [
                    soc_last_day,
                    pd.Series(
                        data=[self.pv_system.soc_now, nan],
        ...
        if not isinstance(self.pv_system.initial_soc, float):
            ...
            self.pv_system.initial_soc = self.pv_system.soc_now
    ...
    def _compare_tariffs(self):
        ...
        base = self.pv_system.flows(solar="solar")
